...
@Component("NEAREST_AVAILABLE")
@RequiredArgsConstructor
public class NearestAvailableStrategy implements PlacementStrategy {

    private final InventoryBalanceRepository balanceRepository;

    @Override
    public List<LocationAllocation> allocate(ProductSku sku, List<Location> eligibleLocations, int requiredQuantity) {
        List<Location> sorted = eligibleLocations.stream()
                .sorted(Comparator.comparing(Location::getSequenceOrder))
                .toList();
        ...
        for (Location loc : sorted) {
            ...
            int available = loc.getCapacity() - balanceRepository.sumQuantityByLocationId(loc.getId());
        ...
        if (remaining > 0) {
            throw new InsufficientCapacityException(sku.getSkuCode());
        }
        return plan;
    }
}
